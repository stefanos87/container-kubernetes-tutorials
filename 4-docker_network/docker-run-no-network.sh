...
docker run -d --name $BACKEND_CONTAINER_NAME -e CONFIGDIR="/config" -e ENDPOINT="BACKEND" -v $HOME_DIR/$CONFIG_DIR:$CONFIG_DIR_MOUNT -p 8082:8081 $BACKEND_IMAGE_NAME:$BACKEND_IMAGE_VERSION
docker run -d --name $ANOTHERBACKEND_CONTAINER_NAME -e CONFIGDIR="/config" -e ENDPOINT="ANOTHER_BACKEND" -v $HOME_DIR/$CONFIG_DIR:$CONFIG_DIR_MOUNT -p 8084:8083 $BACKEND_IMAGE_NAME:$BACKEND_IMAGE_VERSION
docker run -it --name $FRONTEND_CONTAINER_NAME -e BACKEND_ENDPOINT="hello-backend:8081" -e ANOTHER_BACKEND_ENDPOINT="hello-another-backend:8083" -p 8081:8080 $FRONTEND_IMAGE_NAME:$FRONTEND_IMAGE_VERSION