...
# Restaurants Backend
RESTAURANTS_BACKEND_CMD_RUN="docker run -d --name $RESTAURANTS_BACKEND_CONTAINER_NAME --network $CONTAINER_NETWORK -p $RESTAURANTS_BACKEND_CONTAINER_PORT:8082 -v $CONFIG_DIR:$CONFIG_DIR_MOUNT -e CONFIG_DIR=$CONFIG_DIR_MOUNT -e EXPOSED_PORT=$RESTAURANTS_BACKEND_CONTAINER_PORT $RESTAURANTS_BACKEND_IMAGE_NAME:$RESTAURANTS_BACKEND_IMAGE_VERSION"
echo ${cyn}Running Restaurants Backend with:${end} ${grn}$RESTAURANTS_BACKEND_CMD_RUN${end}
$RESTAURANTS_BACKEND_CMD_RUN
# Upload Manager
UPLOAD_MANAGER_CMD_RUN="docker run -d --name $UPLOAD_CONTAINER_NAME --network $CONTAINER_NETWORK -p $UPLOAD_CONTAINER_PORT:8083 -v $UPLOAD_DIR:$UPLOAD_DIR_MOUNT -e UPLOAD_DIR=$UPLOAD_DIR_MOUNT -e EXPOSED_PORT=$UPLOAD_CONTAINER_PORT $UPLOAD_IMAGE_NAME:$UPLOAD_IMAGE_VERSION"
echo ${cyn}Running Upload Manager with:${end} ${grn}$UPLOAD_MANAGER_CMD_RUN${end}
$UPLOAD_MANAGER_CMD_RUN
# Restaurants UI
RESTAURANTS_UI_CMD_RUN="docker run -it --name $RESTAURANTS_UI_CONTAINER_NAME --network $CONTAINER_NETWORK -p $RESTAURANTS_UI_CONTAINER_PORT:80 $RESTAURANTS_UI_IMAGE_NAME:$RESTAURANTS_UI_IMAGE_VERSION"
